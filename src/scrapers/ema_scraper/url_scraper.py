...
ENGLISH_IDENTIFIER = 'english (en)'
SWEDISH_IDENTIFIER = 'svenska (sv)'

def get_links_from_url_en(url):
    """
    Extract the link to the English document

    Argument:
        string url: the url to parse

    Returns:
        string array: all found hrefs
    """
    # GET and parse website
    response = requests.get(url)
    soup = BeautifulSoup(response.content, 'html.parser')

    # Initialize a list to store hrefs to leaflet PDF documents
    hrefs = []

    # Find all <small> tags and filter by text content
    for tag in soup.find_all('small'):
        if ENGLISH_IDENTIFIER in tag.text.strip().lower():
...
def get_links_from_url_sv(url):
    """
    Extract the link to the Swedish document
    ...
    for small_tag in soup.find_all('small'):
        ...
        if SWEDISH_IDENTIFIER in small_tag.text.lower():
            # Parse the relative position of the href, the parent divs next sibling contianing an a-tag
